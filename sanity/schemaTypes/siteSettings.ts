  ...
  fields: [
        ...
        defineField({
          ...
          fields: [
            ...
            defineField({
              ...
              title: 'City',
              type: 'string',
              description: 'City. Used in SEO structured data.',
    ...
    defineField({
      ...
      fields: [
        ...
        defineField({
          name: 'ingredients',
          title: 'Ingredients & allergen information',
          type: 'object',
          description: 'Ingredient and allergen info shown on the Order page, below the form.',
          ...
          fields: [
            defineField({
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.ingredients.heading),
            }),
            defineField({
              name: 'sections',
              title: 'Entries',
              type: 'array',
              description:
                'One entry per cake, frosting, or allergen note (e.g. "Signature Chocolate Cake", "Contains", "Gluten-Free Note"). Add, remove, or reorder freely.',
              of: [
                defineArrayMember({
                  type: 'object',
                  fields: [
        ...
        defineField({
          name: 'form',
          title: 'Order form text',
          type: 'object',
          description: 'Labels, placeholder text, and messages shown on the order form itself.',
          options: {collapsible: true, collapsed: true},
          fields: [
            defineField({
              name: 'chooseCakeLabel',
              title: '"Choose your cake" heading',
              type: 'string',
              description: `Shown above the cake options when there is more than one product. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.chooseCakeLabel),
            }),
            defineField({
              name: 'selectedSummaryLabel',
              title: '"Selected cake summary" label',
              type: 'string',
              description: `Small label above the selected cake's name. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.selectedSummaryLabel),
            }),
            defineField({
              name: 'cakeType',
              title: 'Cake type',
              ...
              fields: [
                defineField({
                  ...
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.cakeType.label),
                }),
                defineField({
                  name: 'regularLabel',
                  title: '"Regular" option label',
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.cakeType.regularLabel),
                }),
                defineField({
                  name: 'glutenFreeLabel',
                  title: '"Gluten-Free" option label',
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.cakeType.glutenFreeLabel),
                }),
              ],
            }),
            defineField({
              ...
              fields: [
                defineField({
                  ...
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.frosting.label),
                }),
                defineField({
                  name: 'options',
                  title: 'Options',
                  type: 'array',
                  description: 'One entry per frosting choice. Add, remove, or reorder freely.',
            ...
            defineField({
              ...
              fields: [
                defineField({
                  ...
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.sprinkles.label),
                }),
                defineField({
                  name: 'options',
                  title: 'Options',
                  type: 'array',
                  description: 'One entry per sprinkles choice. Add, remove, or reorder freely.',
                  of: [
                    defineArrayMember({
                      type: 'object',
                      fields: [
                        ...
                        defineField({
                          name: 'showsNoteField',
                          title: 'Show a note field when selected',
                          type: 'boolean',
                          initialValue: false,
                          description: 'E.g. "Fun Sprinkles" reveals a field asking which colors.',
                        }),
                        defineField({
                          name: 'notePlaceholder',
                          title: 'Note field placeholder',
                          type: 'string',
                          description: 'Only used when "Show a note field" is on.',
                          hidden: ({parent}) => !parent?.showsNoteField,
                        }),
                      ],
                      preview: {
                        select: {title: 'label', subtitle: 'showsNoteField'},
                      },
                    }),
                  ],
                  validation: (rule) => rule.min(1),
            ...
            defineField({
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.nameLabel),
            }),
            defineField({
              name: 'emailLabel',
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.emailLabel),
            }),
            defineField({
              name: 'phoneLabel',
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.phoneLabel),
            }),
            defineField({
            ...
            defineField({
              ...
              fields: [
                defineField({
                  ...
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.fulfillment.label),
                }),
                defineField({
                  name: 'options',
                  title: 'Options',
                  type: 'array',
                  description:
                    'One entry per pickup/delivery zone. Add, remove, or reorder freely.',
                  of: [
                    defineArrayMember({
                      type: 'object',
                      fields: [
                        ...
                        defineField({
                          ...
                          title: 'Requires a delivery address',
                          type: 'boolean',
                          initialValue: false,
                          description: 'Turn on for zones where the customer needs to enter a delivery address.',
                        }),
                      ],
                      preview: {
                ...
                defineField({
                  name: 'addressLabel',
                  title: '"Delivery address" section heading',
                  type: 'string',
                  description: `Shown above the address fields when a delivery zone is selected. ${fallbackHint}`,
                  placeholder: siteFallback(site.order.form.fulfillment.addressLabel),
                }),
              ],
            }),
            defineField({
              name: 'celebratingLabel',
              title: '"What are you celebrating?" field label',
              type: 'string',
              description: `Optional field on the order form. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.celebratingLabel),
            }),
            defineField({
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.instructionsLabel),
            }),
            defineField({
              name: 'instructionsPlaceholder',
              title: 'Special instructions placeholder text',
              type: 'string',
              description: `Grey example text shown inside the empty instructions box. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.instructionsPlaceholder),
            }),
            defineField({
              name: 'hearAboutUs',
              ...
              fields: [
                defineField({
                  ...
                  type: 'string',
                  description: fallbackHint,
                  placeholder: siteFallback(site.order.form.hearAboutUs.label),
                }),
                defineField({
                  name: 'options',
                  title: 'Options',
                  type: 'array',
                  description: 'One entry per option. Add, remove, or reorder freely.',
                  of: [
                    defineArrayMember({
                      type: 'object',
                      fields: [
                        ...
                        defineField({
                          ...
                          type: 'boolean',
                          initialValue: false,
                          description: 'E.g. "Other" reveals a field to specify.',
                        }),
                        defineField({
                          name: 'notePlaceholder',
                          title: 'Note field placeholder',
            ...
            defineField({
              name: 'submitDisclaimer',
              title: 'Note above the submit button',
              type: 'text',
              rows: 2,
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.submitDisclaimer),
            }),
            defineField({
              name: 'submitLabel',
              ...
              type: 'string',
              description: fallbackHint,
              placeholder: siteFallback(site.order.form.submitLabel),
            }),
            defineField({
              name: 'errorMessage',
              title: 'Error message',
              type: 'text',
              rows: 2,
              description: `Shown if the order request fails to send. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.errorMessage),
            }),
            defineField({
              name: 'successHeading',
              title: 'Success heading',
              type: 'string',
              description: `Shown after a successful order request. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.successHeading),
            }),
            defineField({
              name: 'successQuestionsLabel',
              title: 'Success "questions" label',
              type: 'string',
              description: `Text before the email/phone links in the success message. ${fallbackHint}`,
              placeholder: siteFallback(site.order.form.successQuestionsLabel),
            }),
          ],
        }),
      ],
